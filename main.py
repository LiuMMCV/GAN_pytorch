...
## 创建文件夹
os.makedirs("./images/gan1/", exist_ok=True)  ## 记录训练过程的图片效果
os.makedirs("./save/gan1/", exist_ok=True)  ## 训练完成时模型保存的位置
...
for epoch in range(opt["n_epochs"]):  ## epoch:50
    ...
    for i, (imgs, _) in loop:
        ...
        loop.set_postfix(lr=optimizer_G.param_groups[0]['lr'], D_loss=running_loss_D/(i+1), G_loss=running_loss_G/(i+1),
                         D_real=real_scores.data.mean().item(), D_fake=fake_scores.data.mean().item())
